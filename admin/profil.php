<?php
    ...
    if(isset($_POST['kirim-profil'])) {
        ...
        if($fileUpload && trim($_POST['password']) != '') {
            $sql = mysqli_query($conn, "UPDATE tb_admin set foto='$fileFullPath', password='$password' where username='$username'");   
            $foto = $fileFullPath;
        }
        elseif(!$fileUpload && trim($_POST['password']) != '') {
            $sql = mysqli_query($conn, "UPDATE tb_admin set password='$password' where username='$username'");
        }
        elseif($fileUpload && trim($_POST['password']) == '') {
            $sql = mysqli_query($conn, "UPDATE tb_admin set foto='$fileFullPath' where username='$username'");
        }
    }
?>
                            ...
                            <div class="card-body">
                                <form action="profil.php" method="post" novalidate="novalidate" enctype="multipart/form-data">
                                    <div class="form-group">
                                        ...
                                        <input id="cc-pament" name="nama" type="text" class="form-control" aria-required="true" aria-invalid="false" value="<?= $get['nama'] ?>">
                                    </div>
